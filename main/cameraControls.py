...
class Camera:
    ...
    def displayFrame(self , additionalFrames=None , frame=None, windowHeader: str = "Camera Feed", fps: int= None, wait: int=1 ) ->  None:
        """
        ...
        if additionalFrames is not None:
            for index,fr in enumerate(additionalFrames):
                name=f"Header {index}"
                cv2.imshow(name, fr)
